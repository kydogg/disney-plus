...
export async function getAISuggestionYoutube(
	...
	if (!term) {
		return {
			status: 400,
			body: "Please provide a 'term' query parameter",
		};
	}

	try {
		context.log(`Requesting AI suggestions for term: ${term}`);
		...
		const suggestion = completion.choices[0].message.content || "No suggestion";

		context.log(`AI Response received: ${suggestion}`);
